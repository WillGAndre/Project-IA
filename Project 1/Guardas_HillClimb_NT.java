...
class Vertice_HillClimb{
	...
	Vertice_HillClimb(Vertice_HillClimb oVertice){
	...
	public void SetGuarded(Rectangle_HillClimb[] aRectangles){ 
		this.bHasGuard = true; 
	...
	public void UnsetGuarded(Rectangle_HillClimb[] aRectangles){ 
...
class State_HillClimb implements Comparable<State_HillClimb>{
	private LinkedList<Vertice_HillClimb> lGuardedVertices;
	private Rectangle_HillClimb[] aRectangles;
	private int numOfGuards = 0;
	private int maxVerticeIndex = 0;
	private int fitness = 0;

	State_HillClimb(LinkedList<Vertice_HillClimb> lVertices, Rectangle_HillClimb[] aRectangles){
		this.lGuardedVertices = new LinkedList<>();
		for(int i = 0; i < lVertices.size(); i++){
			this.lGuardedVertices.addLast(new Vertice_HillClimb(lVertices.get(i)));
		...
		this.aRectangles = new Rectangle_HillClimb[aRectangles.length];
		for(int i = 0; i < aRectangles.length; i++){
			this.aRectangles[i] = new Rectangle_HillClimb(aRectangles[i]);
		}
		
		this.fitness = this.GetFitness();
	}
    
    public State_HillClimb GetNextState(){
		State_HillClimb oNextState = new State_HillClimb(this.lGuardedVertices, this.aRectangles);

		for(int i = 0; i < this.lGuardedVertices.size(); i++ ) {
			if(!this.lGuardedVertices.get(i).GetGuarded()) {
				Vertice_HillClimb oVertice = this.lGuardedVertices.get(i);
				oVertice.SetGuarded(this.aRectangles);
				State_HillClimb oState = new State_HillClimb(this.lGuardedVertices, this.aRectangles);
				if(oState.GetFitness() > oNextState.GetFitness()){
					oNextState = new State_HillClimb(this.lGuardedVertices, this.aRectangles);
				}
	...
	public LinkedList<State_HillClimb> GetNextStates(){
		int oldFitness = this.fitness;
		int maxFitness = -9999;
		LinkedList<State_HillClimb> lNextStates = new LinkedList<>();
		State_HillClimb nState;
		for(int i = 0; i < this.lGuardedVertices.size(); i++ ) {
			if(!this.lGuardedVertices.get(i).GetGuarded()) {
				this.lGuardedVertices.get(i).SetGuarded(this.aRectangles);
				this.fitness = 0;
				//State_HillClimb nState = new State_HillClimb(this.lGuardedVertices, this.aRectangles);
				int stateFitness = this.GetFitness();
				if(stateFitness > maxFitness) {
					maxFitness = stateFitness;
					lNextStates = new LinkedList<State_HillClimb>();
				}
				if(stateFitness == maxFitness) {
					nState = new State_HillClimb(this.lGuardedVertices, this.aRectangles);
					lNextStates.addLast(nState);
				}
				this.lGuardedVertices.get(i).UnsetGuarded(this.aRectangles);
				if(stateFitness - oldFitness == 9) {
					break;
				}
			}
		}
	...
	public LinkedList<Vertice_HillClimb> GetGuardedVertices(){
		return this.lGuardedVertices;
	}

	public Rectangle_HillClimb[] GetRectangles(){
	...
	@Override
	public int compareTo(State_HillClimb o) {
...
class Field_HillClimb{
	int[][] field;
	char[][] cField;
	int x;
	int y;
	
	Field_HillClimb(LinkedList<Vertice_HillClimb> vertices, Rectangle_HillClimb[] rectangles){
		int maxX = -1;
		int maxY = -1;
		...
		for(int count = 0; count < rectangles.length; count++) {
			Rectangle_HillClimb rec = rectangles[count];
...
public class Guardas_HillClimb {
	
	public static void main(String[] args) {
		Scanner reader = new Scanner(System.in);
		int iNumInstancias = reader.nextInt();
		for(int numInstancia = 0; numInstancia < iNumInstancias; numInstancia++){
			Rectangle_HillClimb[] aRectangles;
			LinkedList<Vertice_HillClimb> lVertices = new LinkedList<Vertice_HillClimb>();

			int iNumRectangles = reader.nextInt();
			
			aRectangles = new Rectangle_HillClimb[iNumRectangles];
			for (int numRectangle = 0; numRectangle < iNumRectangles ; numRectangle++ ) {
				...
				aRectangles[iRectangle] = new Rectangle_HillClimb(iNumVertices);
				...
				for(int numVertices = 0; numVertices < iNumVertices; numVertices++){
					...
					Vertice_HillClimb oVertice;
					if(iVerticePosition == -1){
						oVertice = new Vertice_HillClimb(x,y);
			...
			State_HillClimb state = new State_HillClimb(lVertices,aRectangles);
			LinkedList<State_HillClimb> stateList = new LinkedList<State_HillClimb>();
			int maxFitness = 0;
			...
			while(!stateList.isEmpty()) {
				...
				State_HillClimb newState = stateList.removeFirst();
				System.out.println(newState.GetNumOfGuards());
				LinkedList<State_HillClimb> newStateList = newState.GetNextStates();
				if(!newStateList.isEmpty()) {
					//Com aleatoriedade
					if(newStateList.get(0).GetFitness() > maxFitness) {
						maxFitness = newStateList.get(0).GetFitness();
						Random rand = new Random();
						state = newStateList.get(rand.nextInt(newStateList.size()));
						stateList.addLast(state);
					}
					/* Sem aleatoriedade  
					if(newStateList.get(0).GetFitness() > maxFitness) {
						maxFitness = newStateList.get(0).GetFitness();
						state = newStateList.get(0);
						stateList.addLast(state);
					}
					*/
				}
			...
			long stopTimeAlgo = System.nanoTime();
			System.out.println(((stopTimeAlgo - startTimeAlgo) * 0.000000001));
			//
			//Print Field solution
			int numGuard = 1;
			System.out.println("Instance " + (numInstancia + 1) + ":");
			System.out.println("State_HillClimb Fitness: " + iNumberOfRectanglesToCheck * 3 + " / " + state.GetFitness());
...
//			}
//			System.out.println();
//			System.out.println("Field_HillClimb: ");
//			
//			Field_HillClimb f = new Field_HillClimb(state.GetGuardedVertices(), state.GetRectangles());
//			f.PrintField();
			//
		}
