...
class guards_IDS_GP {
  ...
  //
  static Hashtable<Coor,Integer> initial_coor_Ocorr;
  static boolean initial_arr_Rec[];
  //
  ...
  public static void main(String args[]) {
    ...
    for (int index = 1; index <= numInstan; index++) {
      ...
      initial_coor_Ocorr = (Hashtable) coor_Ocorr.clone();
      initial_arr_Rec = arr_Rec.clone();

      res_insta[index] = str_State();
  ...
  */
  static void checkNewStates(Coor c) {
    ...
    //System.out.println("Pai ("+c.getX()+","+c.getY()+")");

    while (!cand.isEmpty()) {
      Coor c1 = cand.remove();

      //System.out.println("Filhos ("+c1.getX()+","+c1.getY()+")");
        ...
        //
          ...
          //sv_Solut.remove(c,res);
          //sv_Arr.remove(c,arr_Rec);
          //sv_Ocorr.remove(c,coor_Ocorr);
        //
    }
  ...
  */
  ...
  static LinkedList<Coor> str_State() {
    LinkedList<LinkedList> res_optimo = new LinkedList<>();

    LinkedList<Coor> cand = gerirViz(initial_coor_Ocorr, initial_arr_Rec);

    while (!cand.isEmpty()) {
      Coor c1 = cand.remove();

          //garbageColl();
          LinkedList<Coor> tst = IDS(c1,initial_coor_Ocorr,initial_arr_Rec);
          while (tst.isEmpty()) {
            reset(initial_coor_Ocorr,initial_arr_Rec);
            tst = IDS(c1,initial_coor_Ocorr,initial_arr_Rec);
          }
          res_optimo.add(tst);
          reset(initial_coor_Ocorr,initial_arr_Rec);
    }
  ...
  */
  static void garbageColl() { // GB Collection
    stk.clear(); sv_depth.clear();
    sv_Ocorr.clear(); sv_Arr.clear(); sv_Solut.clear();
  }

  /*
    Hard reset - Absolutely necessary
  */
  static void reset(Hashtable<Coor,Integer> hsht, boolean b_arr[]) {
    for (int i = 0; i < numRec; i++)
      b_arr[i] = false;
    // Check for subconjuntos
    readList(hsht,b_arr);
  }

  static LinkedList<Coor> IDS(Coor c1, Hashtable<Coor,Integer> coor_Ocorr, boolean arr_Rec[]) {
    // Empty list
    ...
    while (!stk.isEmpty()) {
      Coor c = stk.pop();
      LinkedList<Coor> res = sv_Solut.get(c);
      boolean arr[] = sv_Arr.get(c);
      Hashtable<Coor,Integer> hash = sv_Ocorr.get(c);

      // IDS
      if (sv_depth.get(c) > maxDepth) {
        maxDepth++;
        return empty;
      }
      ...
      if (checkGoal(arr)) { // Check for goal
        return res;
      }
